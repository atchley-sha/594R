...
Looking at a plot of the emission rates over time, there doesn't appear to be any meaningful trend:
...
site %>% 
	ggplot(aes(x = DATE, y = mean_CO_g_kg)) +
	geom_point() +
	theme_pander() +
	labs(
		x = "Date",
		y = "Mean CO Emissions (g / kg fuel)"
	)
```

Calculating the mean of these values with a 95% confidence interval gives:

```{r}
MeanCI(site$mean_CO_g_kg) %>% 
	bind_rows() %>% 
